...
Deno.serve(async (req) => {
  ...
  for (const contact of targets as any[]) {
    ...
    await settleCredits(admin, {
      ...
      metadata: { contact_id: contact.id, http_status: res.status },
    });
    ...
    if (duplicate?.id) {
      ...
      results.push({ contact_id: contact.id, status: "identity_conflict", existing_contact_id: duplicate.id });
      continue;
    }
    ...
    if (updateErr) {
      results.push({ contact_id: contact.id, status: "crm_update_failed" });
      continue;
    }
    revealed++;
    results.push({ contact_id: contact.id, status: "revealed", organisation_id: contact.organisation_id });
  }
