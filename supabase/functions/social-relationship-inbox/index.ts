...
Deno.serve(async (req) => {
  ...
  if (action === "get_thread") {
    ...
    const [{ data: conv }, { data: messages }] = await Promise.all([
      ...
      a.admin.from("social_relationship_messages").select("*").eq("conversation_id", conversation_id)
  ...
  if (action === "sync_thread") {
    ...
    if (!conv) return json({ ok: false, error: "conversation_not_found" }, 404);
    const { data: account } = await a.admin.from("social_relationship_accounts").select("*").eq("id", conv.account_id).maybeSingle();
    ...
    for (const m of r.data ?? []) {
      ...
      if (!pid) continue;
      const { data: exists } = await a.admin.from("social_relationship_messages").select("id").eq("provider_message_id", pid).maybeSingle();
      if (exists) continue;
  ...
  if (action === "draft_reply") {
    ...
    const { data: msgs } = await a.admin.from("social_relationship_messages").select("*")
      .eq("conversation_id", conversation_id).order("created_at", { ascending: false }).limit(10);
    ...
    if (esc.escalate && !conv.escalation_pending) {
      await a.admin.from("social_relationship_conversations")
        .update({ escalation_pending: true, escalation_reason: esc.category, last_intent: intent }).eq("id", conv.id);
...
Write the next reply only.`,
  ...
  if (action === "queue_reply") {
    ...
    const status = gate.decision === "blocked" ? "blocked" : gate.decision === "ready" ? "approved" : gate.decision === "draft" ? "draft" : "pending_approval";
    ...
    const { data: row } = await a.admin.from("social_relationship_action_queue").insert({
      ...
      approved_by: status === "approved" ? a.user.id : null,
  ...
  if (action === "resolve_escalation") {
    ...
    if (data?.conversation_id) {
      await a.admin.from("social_relationship_conversations")
        .update({ escalation_pending: false, escalation_reason: null }).eq("id", data.conversation_id);
    }
  ...
  if (action === "promote_to_crm") {
    ...
    const { data: existing } = await a.admin.from("social_relationship_crm_links")
      .select("*").eq("profile_id", conv.profile.id).maybeSingle();
    ...
    if (body.crm_contact_id) {
      await a.admin.from("social_relationship_conversations").update({ crm_contact_id: body.crm_contact_id }).eq("id", conversation_id);
    }
