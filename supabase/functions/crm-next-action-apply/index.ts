...
// Requires: founder/admin auth, business-live setting "crm_next_action_creation_enabled" enabled,
// confirmation_phrase === "CREATE CRM NEXT ACTIONS", and dry_run === false.
// NEVER sends email. NEVER calls Apollo. NEVER POSTs to Smartlead.

const CONFIRMATION_PHRASE = "CREATE CRM NEXT ACTIONS";
const SETTING_KEY = "crm_next_action_creation_enabled";
const TARGET_TABLE = "crm_founder_review_queue";
const SOURCE_FUNCTION = "crm-next-action-apply";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = items.slice(0, 100).map((it: any) => ({
      ...
      review_type: String(it.review_type ?? "next_action"),
      priority_level: it.priority_level ?? "normal",
      current_stage: it.current_stage ?? null,
      recommended_stage: it.recommended_stage ?? null,
    ...
    const { data: inserted, error } = await admin.from("crm_founder_review_queue").insert(rows).select("id");
    if (error) {
