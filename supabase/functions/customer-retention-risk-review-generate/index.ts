...
Deno.serve(async (req) => {
  ...
  const confirm = requireConfirmation(body, 'CREATE CUSTOMER RETENTION RISK REVIEW');
  if ((confirm as any).error) return (confirm as any).error;
  if ((confirm as any).dry_run) {
    return json({ ok: true, dry_run: true, no_records_mutated: true, would_insert_into: 'customer_retention_risk_reviews', confirmation_required: 'CREATE CUSTOMER RETENTION RISK REVIEW', ...SAFETY });
  }
  ...
  const { data, error } = await auth.admin.from('customer_retention_risk_reviews').insert(row).select('*').single();
  if (error) return json({ error: error.message }, 400);
  await audit(auth.admin, { business_id: body.business_id, action: 'retention_risk_review_created', after_json: data, is_test_data: row.is_test_data ?? false });
