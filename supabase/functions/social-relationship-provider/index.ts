...
Deno.serve(async (req) => {
  ...
  if (action === "register_webhook") {
    ...
    if (!secret) return json({ ok: false, error: "webhook_secret_missing", hint: "Add SOCIAL_RELATIONSHIP_WEBHOOK_SECRET in Project Settings → Secrets." }, 400);
    const r = await adapter.registerWebhook(callback, secret);
    const conn = await upsertConnection({
  ...
  if (action === "save_policy") {
    ...
    else saved = (await a.admin.from("social_relationship_policies").insert({ business_id, ...patch }).select("*").maybeSingle()).data;
