...
ALTER TABLE public.billionaire_candidate_routes
  ADD COLUMN IF NOT EXISTS route_access_mode text NOT NULL DEFAULT 'unknown_requires_verification',
...
CREATE INDEX IF NOT EXISTS idx_bi_queue_status_priority ON public.billionaire_enrichment_queue (status, priority DESC);
CREATE INDEX IF NOT EXISTS idx_bi_queue_batch_key ON public.billionaire_enrichment_queue (batch_key);
CREATE UNIQUE INDEX IF NOT EXISTS idx_bi_batches_key ON public.billionaire_enrichment_batches (batch_key);
...
DECLARE
  v_key text;
  v_n int;
  v_ver int; v_warm int; v_cand int; v_none int;
BEGIN
  IF NOT public._is_founder_or_admin() THEN
    RAISE EXCEPTION 'not authorised';
  END IF;

  -- reuse an existing in-progress batch, otherwise open the next sequential key
  ...
  -- deterministic selection: unprocessed queue rows only
  ...
  WHERE (q.status IN ('pending','in_progress') OR COALESCE(q.batch_key,'') NOT LIKE 'access_2026_b%')
    AND (q.batch_key = v_key OR q.status IN ('pending','in_progress') OR COALESCE(q.batch_key,'') NOT LIKE 'access_2026_b%')
  ...
  IF v_n = 0 THEN
    RETURN jsonb_build_object('batch_key', null, 'selected', 0, 'done', true);
  END IF;
  ...
  -- 1/2. build or repair candidate routes from existing evidence (idempotent, ranked)
  ...
  -- 3. strict evidence grading of existing pathways for this batch
  ...
  WITH pw AS (
    ...
    WHERE p.route_evidence_state <> 'rejected'
    GROUP BY 1
  ), cand AS (
    ...
    JOIN _batch b ON b.billionaire_id = r.billionaire_id
    GROUP BY 1
  )
  ...
  SET status = CASE
      ...
      last_checked_at = now(), next_check_at = NULL, updated_at = now()
...
CREATE OR REPLACE FUNCTION public.run_billionaire_enrichment_batches(p_max_batches int DEFAULT 10, p_batch_size int DEFAULT 50)
RETURNS jsonb
LANGUAGE plpgsql
SET search_path TO 'public'
AS $function$
DECLARE r jsonb; out jsonb := '[]'::jsonb; i int := 0;
BEGIN
  WHILE i < p_max_batches LOOP
    r := public.process_billionaire_enrichment_batch(p_batch_size);
    EXIT WHEN (r->>'selected')::int = 0;
    out := out || jsonb_build_array(r);
    i := i + 1;
    EXIT WHEN (r->>'remaining')::int = 0;
  END LOOP;
  RETURN jsonb_build_object('batches', out, 'batches_run', i,
    'remaining', (SELECT count(*) FROM public.billionaire_enrichment_queue q
                  WHERE q.status IN ('pending','in_progress') OR COALESCE(q.batch_key,'') NOT LIKE 'access_2026_b%'));
END; $function$;
