...
const TREE_CLEAN = sh(`git status --porcelain -- src supabase apps .github && git diff --name-only ${SOURCE_FREEZE_SHA} -- src supabase apps .github`).split("\n").filter((l) => l.trim() && !/ManualContent|manualArchitectureSync2026/.test(l)).join("").trim() === "";
