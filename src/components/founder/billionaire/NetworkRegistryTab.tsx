    ...
    <div className="space-y-3">
      ...
      <Card className="tech-card p-3">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <Select value={category} onValueChange={setCategory}><SelectTrigger className="text-xs"><SelectValue placeholder="Category" /></SelectTrigger><SelectContent><SelectItem value={ANY}>All categories</SelectItem>{categories.map(x => <SelectItem key={x} value={x}>{x.replace(/_/g, " ")}</SelectItem>)}</SelectContent></Select>
          <Select value={source} onValueChange={setSource}><SelectTrigger className="text-xs"><SelectValue placeholder="Source status" /></SelectTrigger><SelectContent><SelectItem value={ANY}>All source states</SelectItem>{sources.map(x => <SelectItem key={x} value={x}>{x.replace(/_/g, " ")}</SelectItem>)}</SelectContent></Select>
          ...
          <TableBody>
            {filtered.map(r => {
              ...
              return <TableRow key={r.id} className="text-xs">
                <TableCell className="font-medium min-w-[210px]">{r.network_name}<p className="text-[10px] text-muted-foreground mt-1">{r.category.replace(/_/g, " ")}</p></TableCell>
                ...
                <TableCell className="min-w-[200px]">{c ? <><span className="font-medium">{c.person_name ?? "Institutional route"}</span><p className="text-[10px] text-muted-foreground mt-1">{c.role_title ?? c.contact_type.replace(/_/g, " ")}</p></> : <span className="text-yellow-300">Research pending</span>}</TableCell>
                ...
                <TableCell className="max-w-[140px]">{r.access_mode.replace(/_/g, " ")}</TableCell>
