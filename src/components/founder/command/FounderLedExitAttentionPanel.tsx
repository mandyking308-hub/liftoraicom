...
type Counts = { reviewDue: number; prepareForSale: number; awaitingApproval: number; activeOffersOrDiligence: number };
...
export default function FounderLedExitAttentionPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [due, prep, await1, active] = await Promise.all([
