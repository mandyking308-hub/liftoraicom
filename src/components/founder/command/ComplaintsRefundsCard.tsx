...
export default function ComplaintsRefundsCard() {
    ...
    <Card className="tech-card">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px] ml-auto">Live analysis</Badge>
          <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
            <Lock size={9} className="mr-1" /> Refunds approval-gated
          </Badge>
        </CardTitle>
      </CardHeader>
        ...
        {!snap ? <p className="text-xs text-muted-foreground">Calculating…</p> : (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
              <Stat label="Open cases" value={snap.total_open} />
              <Stat label="Critical" value={snap.critical} tone={snap.critical > 0 ? "bad" : "good"} />
              <Stat label="Refunds pending" value={snap.refunds_pending_approval} tone={snap.refunds_pending_approval > 0 ? "warn" : "good"} />
              <Stat label="Legal/disputes" value={snap.legal_or_dispute} tone={snap.legal_or_dispute > 0 ? "bad" : "good"} />
            </div>
            <p className="text-xs text-muted-foreground">{snap.recommended_action}</p>
            <Link to="/founder/complaints" className="text-xs text-primary inline-flex items-center gap-1 hover:underline">
              Open Complaints Engine <ArrowRight size={12} />
            </Link>
          </>
        )}
      </CardContent>
