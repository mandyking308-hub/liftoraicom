...
export default function PortfolioContactRelationshipsTable() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return `${r.email} ${r.name ?? ""} ${r.company ?? ""} ${rel}`.toLowerCase().includes(q);
    });
  }, [rows, search]);
    ...
    <Card className="tech-card">
      <CardContent className="p-4 space-y-4">
        <div className="relative max-w-xl">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search person, organisation or Liftor business" className="pl-9" />
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={4} className="text-center py-6 text-muted-foreground">No matching contacts.</TableCell></TableRow>
              ) : filtered.map((r) => (
                <TableRow key={r.id}>
                  <TableCell>
