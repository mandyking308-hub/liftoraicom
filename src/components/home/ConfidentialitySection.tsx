...
const ConfidentialitySection = () => (
  <section className="py-[120px] max-sm:py-20 section-dark relative overflow-hidden">
    ...
    <div className="container">
      ...
      >
        <motion.div variants={fadeUp} custom={0} className="inline-flex items-center justify-center w-14 h-14 rounded-full border border-primary/20 bg-primary/5 text-primary mb-8">
          <ShieldCheck size={24} strokeWidth={1.5} />
        </motion.div>
        <motion.h2 variants={fadeUp} custom={1} className="text-3xl sm:text-4xl font-bold mb-6">
          Complete Client <span className="text-gradient">Confidentiality</span>
        </motion.h2>
        ...
        <motion.div variants={fadeUp} custom={3} className="mt-10 flex flex-wrap justify-center gap-4">
          {["No public disclosure", "No social media promotion", "Full discretion"].map((item) => (
            <span key={item} className="text-xs font-medium text-primary/70 border border-primary/15 rounded-full px-5 py-2">
              {item}
            </span>
          ))}
        </motion.div>
