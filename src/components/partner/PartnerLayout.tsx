...
import { LayoutDashboard, Lightbulb, FolderKanban, FileText, MessageSquare, LogOut, Menu, X } from "lucide-react";
import { useState } from "react";

const navItems = [
  { label: "Dashboard", to: "/partner", icon: LayoutDashboard },
  { label: "Opportunities", to: "/partner/opportunities", icon: Lightbulb },
  { label: "Projects", to: "/partner/projects", icon: FolderKanban },
  { label: "Documents", to: "/partner/documents", icon: FileText },
  { label: "Messages", to: "/partner/messages", icon: MessageSquare },
];
  ...
  const sidebarContent = (
    <>
      <div className="p-5 border-b border-border/50">
        <Link to="/partner" className="text-lg font-bold tracking-tight">
          <span className="text-foreground">Liftor</span>
          <span className="text-primary"> AI</span>
        </Link>
        <p className="text-xs text-primary mt-0.5 font-medium">Partner Portal</p>
      </div>
