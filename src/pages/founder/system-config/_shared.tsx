...
import { SlidersHorizontal, ArrowLeft, Lock } from "lucide-react";

export const SC_NAV = [
  { to: "/founder/system-config",                     label: "Overview" },
  { to: "/founder/system-config/feature-flags",       label: "Feature Flags" },
  { to: "/founder/system-config/modules",             label: "Modules" },
  { to: "/founder/system-config/external-actions",    label: "External Actions" },
  { to: "/founder/system-config/business-overrides",  label: "Business Overrides" },
  { to: "/founder/system-config/audit",               label: "Audit" },
];

export function SCLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
        ...
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <Lock size={9} className="mr-1" /> External flags gated
            </Badge>
          </h1>
...
export function SCStat({ label, value, hint, tone }: { label: string; value: number|string; hint?: string; tone?: "ok"|"warn"|"bad" }) {
