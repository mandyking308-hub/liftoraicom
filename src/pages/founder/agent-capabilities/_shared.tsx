...
export const ACR_NAV = [
  { to: "/founder/agent-capabilities",                label: "Overview" },
  { to: "/founder/agent-capabilities/registry",       label: "Registry" },
  { to: "/founder/agent-capabilities/boundaries",     label: "Boundaries" },
  { to: "/founder/agent-capabilities/approval-rules", label: "Approval rules" },
  { to: "/founder/agent-capabilities/escalations",    label: "Escalations" },
  { to: "/founder/agent-capabilities/audit",          label: "Audit" },
];

export function AcrLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Agent Capability Registry</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Shield size={20} className="text-primary" />
            {title}
            <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px]">Live control layer</Badge>
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <ShieldAlert size={9} className="mr-1" /> Boundaries enforced
            </Badge>
          </h1>
