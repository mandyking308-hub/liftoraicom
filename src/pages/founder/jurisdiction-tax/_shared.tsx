...
export const JT_NAV = [
  { to: "/founder/jurisdiction-tax",                 label: "Overview" },
  { to: "/founder/jurisdiction-tax/currencies",      label: "Currencies & FX" },
  { to: "/founder/jurisdiction-tax/revenue",         label: "Revenue by currency" },
  { to: "/founder/jurisdiction-tax/customers",       label: "Customer jurisdictions" },
  { to: "/founder/jurisdiction-tax/sellers",         label: "Seller / vendor / entity" },
  { to: "/founder/jurisdiction-tax/adviser-review",  label: "Adviser review" },
  { to: "/founder/jurisdiction-tax/settings",        label: "Settings" },
];

export function JTLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Multi-Currency / Jurisdiction / Tax Tracker</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <Lock size={9} className="mr-1" /> No advice / no filings
            </Badge>
          </h1>
...
export function JTStat({ label, value, hint, tone }: { label: string; value: number | string; hint?: string; tone?: "ok"|"warn"|"bad" }) {
