...
import { Focus, ArrowLeft, ShieldAlert } from "lucide-react";

export const ATT_NAV = [
  { to: "/founder/attention-guard",            label: "Overview" },
  { to: "/founder/attention-guard/today",      label: "Top 10 today" },
  { to: "/founder/attention-guard/noise",      label: "Noise" },
  { to: "/founder/attention-guard/decisions",  label: "Decision fatigue" },
  { to: "/founder/attention-guard/delegation", label: "Delegation" },
  { to: "/founder/attention-guard/settings",   label: "Settings" },
];

export function AttLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Attention Guard</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Focus size={20} className="text-primary" />
            {title}
            <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px]">Live focus protection</Badge>
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <ShieldAlert size={9} className="mr-1" /> Critical risks never hidden
            </Badge>
          </h1>
