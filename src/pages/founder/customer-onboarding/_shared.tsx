...
export const CO_NAV: { to: string; label: string }[] = [
  { to: "/founder/customer-onboarding", label: "Overview" },
  { to: "/founder/customer-onboarding/customers", label: "Customers" },
  { to: "/founder/customer-onboarding/checklists", label: "Checklists" },
  { to: "/founder/customer-onboarding/missing-info", label: "Missing info" },
  { to: "/founder/customer-onboarding/welcome-packs", label: "Welcome packs" },
  { to: "/founder/customer-onboarding/settings", label: "Settings" },
];

export function COLayout({ title, subtitle, children, actions }: { title: string; subtitle?: string; children: ReactNode; actions?: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span>
          <span>Customer Onboarding Engine</span>
        </div>
          ...
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
                <Lock size={9} className="mr-1" /> Customer sends approval-gated
              </Badge>
            </h1>
...
export function COSection({ title, description, children, actions }: { title: string; description?: string; children: ReactNode; actions?: ReactNode }) {
  return (
    <Card className="tech-card">
...
export function COEmpty({ title, hint }: { title: string; hint?: string }) {
  return (
...
export function COStat({ label, value, hint, tone = "default" }: { label: string; value: ReactNode; hint?: string; tone?: "default" | "good" | "warn" | "bad" }) {
...
export const CO_STATUS_TONE: Record<string, string> = {
