...
export const IMPORT_NAV = [
  { to: "/founder/imports",          label: "Overview" },
  { to: "/founder/imports/upload",   label: "Upload" },
  { to: "/founder/imports/mapping",  label: "Mapping" },
  { to: "/founder/imports/preview",  label: "Preview" },
  { to: "/founder/imports/history",  label: "History" },
  { to: "/founder/imports/rollback", label: "Rollback" },
  { to: "/founder/imports/settings", label: "Settings" },
];

export function ImportLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Import / Migration Centre</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Upload size={20} className="text-primary" />
            {title}
            <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px]">Live preview · test by default</Badge>
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <ShieldAlert size={9} className="mr-1" /> Apply requires approval
            </Badge>
          </h1>
