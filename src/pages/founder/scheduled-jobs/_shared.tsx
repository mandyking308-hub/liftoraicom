...
export const SJ_NAV = [
  { to: "/founder/scheduled-jobs",          label: "Overview" },
  { to: "/founder/scheduled-jobs/jobs",     label: "Jobs" },
  { to: "/founder/scheduled-jobs/runs",     label: "Runs" },
  { to: "/founder/scheduled-jobs/failures", label: "Failures" },
  { to: "/founder/scheduled-jobs/calendar", label: "Calendar" },
  { to: "/founder/scheduled-jobs/settings", label: "Settings" },
];

export function SJLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Scheduled Jobs</span>
        </div>
        <div>
...
export function SJStat({ label, value, hint, tone }: { label: string; value: number|string; hint?: string; tone?: "ok"|"warn"|"bad" }) {
