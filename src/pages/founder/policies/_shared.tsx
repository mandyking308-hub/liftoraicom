...
export const POL_NAV = [
  { to: "/founder/policies",               label: "Overview" },
  { to: "/founder/policies/businesses",    label: "Businesses" },
  { to: "/founder/policies/coverage",      label: "Coverage" },
  { to: "/founder/policies/drafts",        label: "Drafts" },
  { to: "/founder/policies/review",        label: "Review queue" },
  { to: "/founder/policies/public-pages",  label: "Public pages" },
];

export function PolLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Policy Coverage</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <ScrollText size={20} className="text-primary" />
            {title}
            <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px]">Live coverage tracking</Badge>
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <ShieldAlert size={9} className="mr-1" /> Publishing approval-gated
            </Badge>
          </h1>
