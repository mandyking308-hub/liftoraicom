...
export const WH_NAV = [
  { to: "/founder/webhooks",                    label: "Overview" },
  { to: "/founder/webhooks/inbox",              label: "Inbox" },
  { to: "/founder/webhooks/providers",          label: "Provider rules" },
  { to: "/founder/webhooks/normalised-events",  label: "Normalised events" },
  { to: "/founder/webhooks/failures",           label: "Failures" },
  { to: "/founder/webhooks/settings",           label: "Settings" },
];

export function WHLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Webhook Inbox</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Inbox size={20} className="text-primary" />
            {title}
            <Badge variant="outline" className="bg-emerald-500/15 text-emerald-400 border-emerald-500/30 text-[10px]">Receive live</Badge>
...
export function WHStat({ label, value, hint, tone }: { label: string; value: number|string; hint?: string; tone?: "ok"|"warn"|"bad" }) {
...
export function procBadge(s: string) {
  const m: Record<string,string> = {
    received: "bg-primary/15 text-primary border-primary/30",
    normalised: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30",
    duplicate: "bg-muted text-muted-foreground border-border/50",
...
export function verBadge(s: string) {
  const m: Record<string,string> = {
    verified: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30",
    not_required: "bg-muted text-muted-foreground border-border/50",
    failed: "bg-red-500/15 text-red-300 border-red-500/30",
