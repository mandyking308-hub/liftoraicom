...
import { ArrowLeft, Radar, Lock } from "lucide-react";

export const DR_NAV = [
  { to: "/founder/distressed-radar",             label: "Overview" },
  { to: "/founder/distressed-radar/acquisition", label: "Acquisition Radar" },
  { to: "/founder/distressed-radar/disposal",    label: "Disposal Shelf" },
  { to: "/founder/distressed-radar/financing",   label: "Financing & Structure" },
  { to: "/founder/distressed-radar/sources",     label: "Weekly Scan Sources" },
];

export function DRLayout({ title, subtitle, actions, children }: { title: string; subtitle?: string; actions?: ReactNode; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Distressed Asset & Brand Radar</span>
        </div>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2 flex-wrap">
              <Radar size={20} className="text-primary" /> {title}
              ...
              <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
                <Lock size={9} className="mr-1" /> Founder approval required for every external action
              </Badge>
            </h1>
...
export function Stat({ label, value, accent }: { label: string; value: number | string; accent?: "amber" | "rose" | "emerald" }) {
