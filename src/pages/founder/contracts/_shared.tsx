...
export const CTR_NAV: { to: string; label: string }[] = [
  { to: "/founder/contracts", label: "Overview" },
  { to: "/founder/contracts/drafts", label: "Drafts" },
  { to: "/founder/contracts/signature", label: "Signature" },
  { to: "/founder/contracts/obligations", label: "Obligations" },
  { to: "/founder/contracts/renewals", label: "Renewals" },
  { to: "/founder/contracts/risk", label: "Risk" },
  { to: "/founder/contracts/settings", label: "Settings" },
];

export function CTRLayout({ title, subtitle, children, actions }: { title: string; subtitle?: string; children: ReactNode; actions?: ReactNode }) {
          ...
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
                <Lock size={9} className="mr-1" /> Send / sign / terminate approval-gated
              </Badge>
            </h1>
...
export function CTRSection({ title, description, children, actions }: { title: string; description?: string; children: ReactNode; actions?: ReactNode }) {
  return (
    <Card className="tech-card">
...
export function CTREmpty({ title, hint }: { title: string; hint?: string }) {
  return (
...
export function CTRStat({ label, value, hint, tone = "default" }: { label: string; value: ReactNode; hint?: string; tone?: "default" | "good" | "warn" | "bad" }) {
...
export const CTR_STATUS_TONE: Record<string, string> = {
...
export const CTR_OBL_TONE: Record<string, string> = {
  pending: "bg-blue-500/15 text-blue-400 border-blue-500/30",
  in_progress: "bg-blue-500/15 text-blue-400 border-blue-500/30",
  completed: "bg-emerald-500/15 text-emerald-400 border-emerald-500/30",
  overdue: "bg-red-500/15 text-red-400 border-red-500/30",
  waived: "bg-muted text-muted-foreground border-border/50",
  cancelled: "bg-muted text-muted-foreground border-border/50",
};

export const CTR_RISK_TONE: Record<string, string> = {
