...
export const BR_NAV = [
  { to: "/founder/backup-recovery",                label: "Overview" },
  { to: "/founder/backup-recovery/status",         label: "Backup Status" },
  { to: "/founder/backup-recovery/exports",        label: "Exports" },
  { to: "/founder/backup-recovery/restore",        label: "Restore Checklists" },
  { to: "/founder/backup-recovery/emergency-pack", label: "Emergency Pack" },
  { to: "/founder/backup-recovery/settings",       label: "Settings" },
];

export function BRLayout({ title, subtitle, children }: { title: string; subtitle?: string; children: ReactNode }) {
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <span>/</span><span>Backup / Export / Recovery</span>
        </div>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            <Badge variant="outline" className="bg-yellow-500/15 text-yellow-300 border-yellow-500/30 text-[10px]">
              <Lock size={9} className="mr-1" /> Exports/Restore gated
            </Badge>
          </h1>
...
export function BRStat({ label, value, hint, tone }: { label: string; value: number|string; hint?: string; tone?: "ok"|"warn"|"bad" }) {
