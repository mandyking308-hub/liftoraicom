...
export const LIFTOR_USER_MANUAL_VERSION = "1.11 — Education Commercial Layer (10 September 2026)";
...
export const ARCHITECTURE_SYNC_USER_SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    body: "Read this before you touch outreach. WORKING AND SAFE: everything internal — Command Centre, Daily Operator, business setup and onboarding, knowledge and the new source-manifest/fidelity check, drafting, approvals, finance and reporting review, and free Apollo people search that only stages candidates for review. STILL GATED, DO NOT START YET: paid Apollo enrichment (there is no portfolio-wide credit limit built yet, so automatic paid reveal stays off) and live Smartlead sending. Smartlead is connected and a lot of the plumbing exists, but the live check found no campaign links, no lead links, no events and no completed activation checklist — so the loop is not finished and nothing should be pushed or started. Smartlead is how emails would be delivered; it is not where education prospect data comes from. The 8 sending domains and 2 mailboxes on file are old Neon Candy infrastructure, not the planned education sending setup. Your working CRM (81 people, 68 business links) is a different thing from Relationship Intelligence research lists. And the 2,520-row education list from August is historical evidence only — the live check on 10 September found none of it in Relationship Intelligence, so treat restoring it as its own deliberate job, not a done deal. NEW SINCE THIS UPDATE: the credit safety wall for Apollo is now built. Every paid Apollo lookup anywhere in the system has to ask permission from one shared budget first, and that budget is currently set to zero with paid lookups switched off, so nothing can spend money by accident. The tools for loading the 152 education groups and for finding decision-makers using the free search are also built, but they are set to preview-only and the 152 groups have not been loaded yet. There is a new read-only panel on the Apollo page showing the spend wall, the budget and how much education data exists. Nothing on that panel can spend money or send an email. CORRECTION (CRM-native education placement): education companies are now stored once in your main company list, and education people are stored once in your main CRM, attached to their company. The research view links back to the same company record instead of holding a second copy. Research people are created with no email and marked not sendable, so nothing can be sent to them. A separate founder-only 'reveal business email' step exists for people you personally select, but it cannot run at all while the Apollo budget wall is locked at zero.",
  },
  {
    number: 112,
    key: "education-commercial-layer-user",
    title: "112. The four education businesses and how they share one contact list",
    body: "You now have four education businesses set up in Liftor: Billy and the Wild Forest, Aurelia, Kindnesss and Kingsbridge Global. Neon Candy is completely separate and was not touched. Each person is still stored only once, attached to their school or group. What changes per business is a link on the side saying how relevant that person is to that brand and why — so one head of inclusion can be relevant to Billy and to Kindnesss without ever creating two copies of them. Billy scores SEN, SEND and inclusion people highest, then literacy, wellbeing, curriculum and leadership. Only one brand may talk to a person at a time. If a brand takes a person, the others are locked out until it lets go, and then there is a 30-day pause before another brand may approach. If someone replies, is unsubscribed, has asked not to be contacted, is suppressed or has bounced, every brand is blocked and you cannot override that — the override only exists for deciding which brand goes first. Four campaign shells are ready and all four are switched off: nothing is approved, nothing is live, sending is blocked and no shell is linked to a sending campaign. Billy's is the one prepared first, for a careful opening batch of 25 to 50 excellent contacts — not blind hundreds. Before anything can go out, a person must have a school attached, a qualifying brand link, a clean safety record, no clash with another brand, a usable work email, an approved and live campaign, and both the sending setup and the mailboxes ready. Those last two are handled separately and are not ready, so nothing can send today. You can see all of this at Opportunity → Education Commercial Layer, and each business has three manuals (technical, how-to and a customer-facing one) at Opportunity → Business Manuals. None of this changed anything about Apollo scoring or the Apollo spend wall.",
  },
];
