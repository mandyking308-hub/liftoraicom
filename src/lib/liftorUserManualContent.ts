...
export const LIFTOR_SIMPLE_GUIDE: ManualSection[] = [
  ...
  { number: 27, key: "source-manifest-fidelity", title: "Standard low-credit onboarding: source manifest → fidelity → training → starter pack → activation", body: "This is the standard, lowest-cost way to onboard a business. 1) Take the business's own source of truth (Lovable project manifest, uploaded manual, pasted manifest, GitHub manifest or website manifest) and paste it as ONE snapshot into Command Centre → Business Knowledge card → 'Source manifest & fidelity' → Register snapshot. It is stored as a durable knowledge source (upload_type=source_manifest) with source_ref, source_project_id, source_version, source_hash and last_synced_at. Liftor does NOT continuously poll the source — you re-register a snapshot when the source changes. 2) Click 'Run fidelity check'. Liftor deterministically compares the manifest against its own derived understanding (knowledge profile + starter pack) on critical fields: purpose, ICP, offers, pricing, brand constraints and approval rules. Missing source data is reported as MISSING / NEEDS SOURCE and is never invented; missing derived data is reported as NOT FOUND. 3) Verdicts: FIDELITY_PASS, FIDELITY_REVIEW, or FIDELITY_FAIL. A FIDELITY_FAIL means the source and Liftor's stored understanding contradict each other on a critical field (for example the source says education operations but the stored knowledge says logistics) — internal activation is blocked until founder review. 4) Then run training, generate the starter pack, and only then activate internally. Nothing in this route sends, publishes or contacts anyone." },
...
const TECHNICAL_MANUAL_LIVE_STATE: ManualSection = {
  ...
  body: [
    ...
    "Database tables (audited live, 17 September 2026). 1,112 public tables — 1,109 with rowsecurity=true and 3 with RLS OFF (open finding, see the Rebuild Manual Section R1). Hot paths: ai_usage_ledger, ai_gateway_requests, ai_runtime_events, ai_provider_pricing, ai_business_budgets, ai_agent_cost_controls, ai_kill_switch_state, founder_approval_items, external_action_gates, execution_result_log, business_*, brain_*, apollo_*, agent_*.",
...
export const ARCHITECTURE_SYNC_USER_SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    body: "Read this before you touch outreach. WORKING AND SAFE: everything internal — Command Centre, Daily Operator, business setup and onboarding, knowledge and the new source-manifest/fidelity check, drafting, approvals, finance and reporting review, and free Apollo people search that only stages candidates for review. STILL GATED, DO NOT START YET: paid Apollo enrichment (the portfolio-wide Apollo firewall exists and remains locked with paid enrichment OFF and hard limit 0) and live Smartlead sending. Smartlead is connected and a lot of the plumbing exists, but the live check found no campaign links, no lead links and no events; the activation checklist now exists and truthfully shows the remaining blockers (see Section 115) — so the loop is not finished and nothing should be pushed or started. Smartlead is how emails would be delivered; it is not where education prospect data comes from. The 8 sending domains and 2 mailboxes on file are old Neon Candy infrastructure, not the planned education sending setup. Your working CRM (the live people and business-link counts shown on the CRM page) is a different thing from Relationship Intelligence research lists. And the 2,520-row education list from August is historical evidence only — the live check on 10 September found none of it in Relationship Intelligence, so treat restoring it as its own deliberate job, not a done deal. NEW SINCE THIS UPDATE: the credit safety wall for Apollo is now built. Every paid Apollo lookup anywhere in the system has to ask permission from one shared budget first, and that budget is currently set to zero with paid lookups switched off, so nothing can spend money by accident. The tools for loading the 152 education groups and for finding decision-makers using the free search are also built and set to preview-only. The 152 reviewed education groups are now loaded once into the canonical CRM organisations and mirrored to the strategic planning layer, split as 60 International operator, 54 Domestic reserve, 21 Network route and 17 Review needed. No education people were replayed by this company load. There is a new read-only panel on the Apollo page showing the spend wall, the budget and how much education data exists. Nothing on that panel can spend money or send an email. CORRECTION (CRM-native education placement): education companies are now stored once in your main company list, and education people are stored once in your main CRM, attached to their company. The research view links back to the same company record instead of holding a second copy. Research people are created with no email and marked not sendable, so nothing can be sent to them. A separate founder-only 'reveal business email' step exists for people you personally select, but it cannot run at all while the Apollo budget wall is locked at zero. Live Smartlead sending remains gated.",
  },
  {
    number: 112,
    key: "sending-infrastructure-user",
    title: "112. Sending infrastructure — what is built and what you must do tomorrow",
    body: "Sending Infrastructure (/founder/sending-infrastructure) shows the exact 12-item readiness checklist, mailbox estate, bulk mailbox preview/apply, readiness refresh, zero-provider-mutation send dry run and read-only provider mailbox discovery. Nothing on that page sends an email or starts a provider campaign. Current truth as of 17 September 2026: the estate is real and reconciled — 39 sending domains, all with verified DNS, SPF, DKIM and DMARC, and 200 mailboxes, all active and all warming. But Smartlead is connected only to the 10 trust mailboxes, none of your 180 shared-estate mailboxes is connected, the webhook is still not configured, there are no campaign links, lead links or returned events, and zero mailboxes are campaign-ready. Having 200 warming mailboxes is inventory, not permission to use 200. The next step is not to connect everything: it is one tiny controlled pilot with one business, a handful of recipients and one or two chosen senders, with every other mailbox kept out. Only after that pilot genuinely works end to end do you consider your live-launch approval."
  },
  {
    number: 113,
  ...
  },
  {
    ...
    body: "GSM Outbound Infrastructure (/founder/gsm-outbound) is the shared GSM-owned sender estate; the trust estate has its own page at /founder/ghat-outbound and is never mixed with it. As of 17 September 2026 the provider estate has been reconciled into Liftor: 39 domains and 200 mailboxes in total, all DNS-verified and all warming, made up of 180 shared-estate mailboxes across 36 domains and 20 trust mailboxes across 3 domains. Businesses borrow capacity; they do not own mailboxes, and sticky conversations keep their original sender. A mailbox is still not usable merely because it exists and is warming: SMTP and IMAP must be healthy, Smartlead must be connected, warm-up must be complete, health must pass and a safe daily cap must exist. Today zero mailboxes meet that bar. The old hello@neoncandy.online mailbox is permanently outside this estate. Pilot-first rule: only a very small number of senders may ever be allocated until one controlled pilot has been delivered and returned successfully."
  },
  {
    number: 115,
    key: "smartlead-activation-truth",
    title: "115. Sending activation — exactly where it stands now",
    body: "The Smartlead activation checklist is live and contains exactly 12 controls for each of Billy and the Wild Forest, Aurelia, Kindnesss and Kingsbridge Global. The rehearsal function is fail-closed: missing or synthetic details return BLOCKED and make no provider call and no database change. As of 17 September 2026 the physical sending domains and mailboxes exist and are warming, so that part is no longer the blocker. What is still missing is the proof: your shared-estate mailboxes are not connected in Smartlead, the webhook secret and endpoint are not configured, no campaign shell is bound to a provider campaign, no lead has been pushed and no event has come back. The four education campaign shells remain switched off, externally blocked, unapproved and unmapped, and Neon Candy stays separate. Nothing goes live until one tiny pilot has been proved and you then explicitly approve live launch."
  },
  {
    number: 116,
  ...
  },
  {
    ...
    title: "117. Finding any founder page — every route now has an operator entry",
    body: "Every one of Liftor's 798 distinct founder pages is now documented, with none left as 'listed but unexplained'. 191 pages have their own entry: this manual names some of them directly, and each of the 190 module families has a generated operator entry in Appendix W of the Rebuild Manual (docs/liftor-rebuild/W-module-operator-entries.md) giving the page to open at, what you see on it, its controls, whether it writes anything, which backend functions it can reach ('none' means it cannot reach any outside provider at all), the exact approval phrase if one guards it, and its empty or error states. The other 607 pages are sub-tabs or detail views: Appendix V (docs/liftor-rebuild/V-user-manual-coverage.md) gives every single one of them a row naming the exact parent page you operate it from, the relationship, the source file behind it, whether it writes, its backend functions and any approval phrase. Nothing is classified as legacy, dead or internal-only, because no founder route qualifies. Universal rule: internal reads and drafts are safe; anything that asks you to type a confirmation phrase is an external action and stays blocked unless the matching gate is explicitly enabled.",
  },
];
