...
export const ARCHITECTURE_SYNC_USER_SECTIONS: ManualSection[] = [
  ...
  },
  {
    ...
    title: "115. Sending: exactly where it stands and what is still missing",
    body: "Update to Section 111: there is now a proper activation checklist saved for each of your four education businesses. It has twelve items and it is filled in from real live state, not guesses. Right now every business shows the same picture: two items done (the sending provider is connected, and the rule that blocks anyone who replied, unsubscribed, bounced or is on do-not-contact is enforced everywhere), and ten items not done. The ten missing ones are: the provider is not yet told where to send replies back to us, no campaign is linked, no people are linked, the reply loop is therefore not proven, there are no sending domains, no company mailboxes, nothing warmed up, no daily limits set, no successful end-to-end rehearsal, and — importantly — you have not given live launch approval. That last one stays switched off until you personally say so. The rehearsal tool works and, when asked to imagine sending, it stops and says BLOCKED and explains at which step, without touching the provider or the database. Nothing sends today, no campaign has been started in the sending provider, and hello@neoncandy.online is still fenced off and untouched. What only you can do next, in the real world: buy and verify the sending domains, create the mailboxes and start them warming, connect them to the sending provider, and set up the reply webhook with a secret password that I will store for you. After that the checklist will start turning green on its own.",
  },
];
