...
**Canonical current-architecture sections:** SECTION 100 (August 2026 whole-platform reconciliation) and SECTION 101 (10 September 2026 reconciliation delta) at the end of this manual. Where they conflict on *current* state, SECTION 101 controls, and SECTION 105 (17 September 2026 outbound readiness baseline) controls on all *outbound* current state.
**17 September 2026 outbound truth in one paragraph:** the physical sending estate is now reconciled and real — 39 sending domains, all DNS/SPF/DKIM/DMARC verified, and 200 mailboxes, all active and warming, split GSM 180 mailboxes across 36 domains and GHAT 20 mailboxes across 3 domains. Only 10 mailboxes (GHAT) are connected to Smartlead; 0 GSM mailboxes are connected. **0** mailboxes are campaign-ready, and there are **0** campaign mappings, **0** lead mappings and **0** returned provider events. The Smartlead provider row is connected and healthy but \`webhook_configured = false\` and \`warmup_status = not_configured\`. Apollo infrastructure exists with only the Neon Candy connection row active and verified; Apollo is **not** yet configured business-by-business across the portfolio. There are **14** business rows today — the portfolio is intended to grow through the Business Setup Tunnel / Business Onboarding Factory, so never claim 40 businesses are configured. **200 registered warming mailboxes are inventory, not permission to use 200:** outbound must first be proved by a tiny controlled pilot (one business, ≤5 recipients under the existing micro-batch cap, one or two selected senders, every other mailbox excluded from allocation) that runs ICP/offer/copy → data import → canonical CRM + business relationship → suppression/sendability → selected sender → campaign/lead mapping → delivery → reply/bounce/unsubscribe return → CRM state update → audit trail, failing closed at any broken step. **No pilot has been sent and none has passed.** Founder final live-launch approval remains a separate gate after the pilot, and estate-wide activation a further gate after that. Full detail: SECTION 105.
**Earlier September 2026 truth in one paragraph (superseded on outbound state by SECTION 105):** Apollo free People Search, staging, qualification and selective founder-approved reveal work, but Neon Candy-specific defaults remain in several search/quality paths and there is **no portfolio-wide Apollo credit firewall yet** — automatic paid enrichment stays OFF. Smartlead is **connected** with substantial code implemented, but the audited live state shows **0 campaign mappings, 0 lead mappings, 0 provider events and 0 activation-checklist rows**; Smartlead is the *delivery* lane, not the preferred education prospect-data source. The current 8 sending domains / 2 inboxes are **legacy Neon Candy infrastructure**, not the planned education sending estate. The operational CRM (81 contacts, 68 business relationships) is distinct from Relationship Intelligence and the research pools. The historical 2,520-row education recovery is **evidence only** — the 10 September live check found **zero** \`school_education_contact\` rows in \`relationship_intelligence_contacts\`, so it must not be quoted as a current live count. A business **source-manifest / source-fidelity** layer has been added since August. **Stage 4 is now implemented:** a portfolio-wide Apollo Credit Firewall guards every paid Apollo call, the education 152 account importer and the free-search education discovery orchestrator exist, and a read-only firewall status panel is on the Apollo page. Paid enrichment nevertheless remains **disabled** with a hard portfolio credit limit of **0**, phone/personal-email/waterfall reveal all false, and the 152 education accounts are **not yet imported**. A **CRM-native correction (Section 101.11)** now supersedes the earlier Stage-4 placement wording: \`organisations\` is the canonical education company spine, \`contacts\` (with a real \`organisation_id\`) is the canonical person registry, \`strategic_target_accounts\` links back via \`existing_organisation_id\`, and Relationship Intelligence is research/evidence only. A founder-controlled selected reveal path (\`apollo-education-reveal-selected\`) exists but is unusable while the firewall is locked at 0. Build discipline is **GitHub-first**: \`main\` is the code source of truth and every architecture change ships its manual update in the same build unit.
