...
**Final hardening (production-grade):**
- **Domain reputation shield**: `sending_domains.domain_reputation_score` is the average of all active inbox scores on that domain (auto-recomputed via `recompute_domain_reputation` + `trg_sync_domain_reputation`). `check_send_throttle` halves effective daily limit when domain score < 40 and pauses sending entirely when < 20 (`DOMAIN_REPUTATION_PAUSE`).
- **Send variance**: `outreach-send-worker` sleeps a random 30s–180s between sends (skipped on first item) to avoid pattern detection.
- **Reply-first priority**: `email_queue.priority` (default 100, lower = higher priority). Trigger `set_queue_priority_on_reply` sets priority=10 on insert when contact replied within 14 days. Worker orders by priority ASC then scheduled_at. `check_send_throttle` returns `REPLY_PRIORITY_BYPASS` for replied contacts, skipping window/throttle checks (still respects hard reputation pauses).
- **Soft-fail retry**: `email_queue.retry_count` + `last_attempt_at`. `mark_send_failure(queue_id, error)` schedules retries with exponential backoff (5min → 20min → 60min) and only flips to `failed` after 3 attempts. Worker calls this on send exceptions instead of failing hard.
- **Timezone confidence**: `contacts.timezone_confidence` enum (high/medium/low). Trigger `set_timezone_confidence` auto-sets it: explicit timezone=high, country only=medium, neither=low. Throttle widens window for low-confidence contacts (7-19 vs 8-17).
- **Inbox rotation memory**: `inboxes.last_used_sequence_position` (round-robin) + `consecutive_failures`. `pick_inbox_for_business(business_name)` picks the inbox with the lowest `last_used_sequence_position`, ties broken by reputation DESC then current_send_count ASC, then stamps the next position so the same inbox doesn't always start campaigns.
